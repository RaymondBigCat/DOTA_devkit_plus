...
def custombasename(fullname): # 返回文件名（不带后缀）
    return os.path.basename(os.path.splitext(fullname)[0])

def GetFileFromThisRootDir(dir,ext = None): # 取出该路径下所有文件的绝对路径
    allfiles = []
...
'''
统一parse模式，将parseMode作为一个参数传入
parseMode代表了不同的标注解析模式
'''
def parse_dota(filename, parseMode = 'parse_dota_poly'):
    if parseMode == 'parse_dota_poly':
        objects = parse_dota_poly(filename) 
                    
    # 解析格式：[x1, y1, x2, y2, x3, y3, x4, y4]
    elif parseMode == 'parse_dota_poly2':
        objects = parse_dota_poly2(filename)

    # 解析格式："xmin, ymin, xmax, ymax"
    elif parseMode == 'parse_dota_rec':
        objects = parse_dota_rec(filename)
               
    # 解析格式：[cx, cy, w, h, theta]
    elif parseMode == 'parse_dota_rot_rect':
        objects = parse_dota_rot_rect(filename)
    else:
        print(" Error: This parseMode is not supported \n")
        print(" Try the default parseMode instead \n")
        objects = parse_dota_poly(filename)
    return objects
...
def parse_dota_poly2(filename): # 将ground truth解析成另一种格式
    """
        parse the dota ground truth in the format:
        [x1, y1, x2, y2, x3, y3, x4, y4]
    """
...
def polygonToRotRectangle(bbox): # 将四点框转化为旋转框 ！
    """
    ...
    center = np.array(center,dtype=np.float32)/4.0 # 中心点直接就是粗暴的原始坐标均值
...
# 从filename读入annotation，并解析到旋转框
def parse_dota_rot_rect(filename):
    objects = parse_dota_poly2(filename)
    for obj in objects:
        obj['poly'] = polygonToRotRectangle(obj['poly'])
        # obj['poly'] = list(map(int, obj['poly']))
    return objects
